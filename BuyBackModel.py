...
def OptimumBatterySize(): # Determines the optimum battery size
    return int(round(ActualExcessPower(), -3) / 1000) # If you want a set battery size just hash the return and just return your size in MW
...
def BuybackSaving(): # Creates lists of separate data
    ...
    return lifetimelist, degradation, savinglistday, savinglistyear, accumalatedsaving, councilsaving, totalcouncilsaving, totalemissions, accumemissions

def PrintBuyBack(): # Prints the data in a DataFrame Structure
    lifetimelist, degradation, savinglistday, savinglistyear, accumalatedsaving, councilsaving, totalcouncilsaving, totalemissions, accumemissions = BuybackSaving()
    df = pd.DataFrame({
    'Year':lifetimelist,
